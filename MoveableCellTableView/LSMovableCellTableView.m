...
#import "LSMovableCellTableView.h"

static NSTimeInterval kJXMovableCellAnimationTime = 0.25;

@interface LSMovableCellTableView ()
...
@implementation LSMovableCellTableView
...
- (void)setCanMoveSection:(NSInteger)canMoveSection {
    _canMoveSection = canMoveSection;
}
...
- (void)jx_gestureBegan:(UILongPressGestureRecognizer *)gesture
{
    ...
    // 如果不是设置的可移动section, 直接返回
    if (self.canMoveSection >= 0) {
        if (selectedIndexPath.section != self.canMoveSection) {
            return;
        }
    }
...
- (void)jx_gestureChanged:(UILongPressGestureRecognizer *)gesture
{
    ...
    if (self.canMoveSection != 0) {
        if (currentIndexPath.section == self.canMoveSection) {
            //让截图跟随手势
